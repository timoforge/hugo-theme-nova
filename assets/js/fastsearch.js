...
const buildFuseOptions = () => {
  ...
  return {
    ...
    ignoreLocation:
      params.fuseOpts.ignorelocation ?? defaultFuseOptions.ignoreLocation,
  };
};
...
const performSearch = () => {
  ...
  const results = searchOptions
    ? fuse.search(query, searchOptions)
    : fuse.search(query);
...
document.addEventListener("keydown", (event) => {
  ...
  if (key === "ArrowDown") {
    ...
    if (active === sInput) {
      ...
      setActiveResult(
        active?.parentElement?.nextElementSibling?.querySelector(".entry-link")
      );
    }
  } else if (key === "ArrowUp") {
    event.preventDefault();
    ...
    if (active?.parentElement === firstResult) {
      ...
      setActiveResult(
        active?.parentElement?.previousElementSibling?.querySelector(
          ".entry-link"
        )
      );
    }
